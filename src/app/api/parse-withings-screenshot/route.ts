...
const EXTRACTION_PROMPT = `You are reading one or more screenshots of Withings Sleep Analyzer (or Withings Health Mate app) sleep results. Each image may be from a DIFFERENT night — do NOT merge them together. Process each image independently and return one result object per image, in the same order the images were provided.

For each image, extract the values EXACTLY as shown — do not estimate or infer anything not visible on screen. Also report the date the screen is showing (relative label like "Today"/"Yesterday", or a specific date/weekday). Report whatever text is shown exactly. If genuinely no date indicator is visible, use null.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Accept either a single image (legacy) or an array of images (multiple
    // nights, or a full night's Withings results spread across several screens).
